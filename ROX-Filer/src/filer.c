...
static void filer_restore_default_cursor(GdkWindow *window)
{
    if (!window)
        return;

#ifdef GDK_WINDOWING_WAYLAND
    /* On Wayland, explicitly restore the normal pointer instead of sending a
     * NULL cursor. This avoids leaving the seat with no visible cursor when
     * pointer focus moves between ROX and clients that do not immediately set
     * their own cursor on enter. Preserve the historical NULL/inherited
     * cursor behaviour on X11/XLibre. */
...
void filer_init(void)
{
	...
	default_cursor = gdk_cursor_new_from_name(gdk_display_get_default(), "default");
	if (!default_cursor)
	...
	if (fn != old_fn)
		{
				...
				{
					if (fn)
						gdk_window_set_cursor(view_window, crosshair);
					else
						filer_restore_default_cursor(view_window);
				}
			}
