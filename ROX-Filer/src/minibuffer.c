...
static void minibuffer_beep(GtkWidget *widget);
...
/* GTK3: emit the bell through the display associated with the widget. */
static void minibuffer_beep(GtkWidget *widget)
{
	GdkDisplay *display;
...
void create_minibuffer(FilerWindow *filer_window)
{
	...
	/* Modificado por josejp2424: GTK3 ya gestiona correctamente grab-focus. */
...
static void path_return_pressed(FilerWindow *filer_window, GdkEventKey *event)
{
	...
	{
		minibuffer_beep(filer_window->minibuffer);
		return;
	}
...
static void complete(FilerWindow *filer_window)
{
	...
	else
	{
		...
		new = (const gchar *) make_path(filer_window->sym_path,
						item->leafname);

		if (item->base_type == TYPE_DIRECTORY &&
				(item->flags & ITEM_FLAG_APPDIR) == 0)
			new = (const gchar *) make_path(new, "");
	...
	switch (filer_window->mini_type)
	{
		...
		case MINI_SELECT_IF:
			set_find_string_colour(GTK_WIDGET(mini),
				(const guchar *) gtk_entry_get_text(
