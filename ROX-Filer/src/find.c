...
static gboolean test_xattr(FindCondition *condition, FindInfo *info)
{
	...
	const char *value = condition->data2;

	switch (type)
	{
		case X_ATTR:
		{
			gchar *attribute = xattr_get((const char *) info->fullpath,
						     value, NULL);
		...
		case X_LABEL:
		{
			...
			if (!gdk_rgba_parse(&requested, value))
				break;

			actual = xlabel_get((const char *) info->fullpath);
			if (actual)
			{
				matches = gdk_rgba_equal(actual, &requested);
				gdk_rgba_free(actual);
			}
			return matches;
		}
	}
...
static Eval *parse_eval(const gchar **expression)
{
	...
	eval->data1 = g_memdup2(&value, sizeof(value));
