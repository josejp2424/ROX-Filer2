...
/* Static prototypes */
static void draw_one_item(Collection *collection, cairo_t *cr,
			  int item, GdkRectangle *area);
...
static void collection_dispose(GObject *object);
static void collection_finalize(GObject *object);
static void collection_map(GtkWidget *widget);
static void collection_get_preferred_width(GtkWidget *widget,
					   gint *minimum, gint *natural);
static void collection_get_preferred_height(GtkWidget *widget,
					    gint *minimum, gint *natural);
...
static void collection_set_property(GObject      *object,
				    ...
				    const GValue *value,
				    GParamSpec   *pspec);
static gboolean collection_draw(GtkWidget *widget, cairo_t *cr);
static void default_draw_item(GtkWidget *widget, cairo_t *cr,
				CollectionItem *data,
				GdkRectangle *area,
				gpointer user_data);
...
static void draw_lasso_box(Collection *collection, cairo_t *cr);
static void queue_lasso_redraw(Collection *collection);
...
static void draw_focus_at(Collection *collection, cairo_t *cr,
                          GdkRectangle *area)
{
    ...
    gtk_style_context_set_state(context,
            gtk_widget_has_focus(widget) ? GTK_STATE_FLAG_FOCUSED
                                         : GTK_STATE_FLAG_INSENSITIVE);
    gtk_render_focus(context, cr, area->x, area->y,
                     collection->item_width, area->height);
                          ...
                          int item, GdkRectangle *area)
{
    ...
    {
        collection->draw_item(GTK_WIDGET(collection), cr,
                              &collection->items[item], area,
                              collection->cb_user_data);
    }

    if (item == collection->cursor_item)
        draw_focus_at(collection, cr, area);

    if (item == collection->wink_item && collection->wink_visible)
    {
        const double dash[] = { 3.0, 3.0 };

        cairo_save(cr);
        cairo_set_source_rgba(cr, 0.95, 0.35, 0.10, 0.95);
        cairo_set_line_width(cr, 2.0);
        cairo_set_dash(cr, dash, G_N_ELEMENTS(dash), 0);
        cairo_rectangle(cr, area->x + 1.0, area->y + 1.0,
                        collection->item_width - 3.0, area->height - 3.0);
        cairo_stroke(cr);
        cairo_restore(cr);
    }
}
...
static void collection_class_init(GObjectClass *gclass, gpointer data)
{
    ...
    GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(gclass);

    parent_class = g_type_class_peek_parent(gclass);

    object_class->dispose = collection_dispose;
    object_class->finalize = collection_finalize;

    widget_class->draw = collection_draw;
    widget_class->get_preferred_width = collection_get_preferred_width;
    widget_class->get_preferred_height = collection_get_preferred_height;
    ...
    collection_signals[GAIN_SELECTION] = g_signal_new("gain_selection",
                    G_TYPE_FROM_CLASS(gclass), G_SIGNAL_RUN_LAST,
                    G_STRUCT_OFFSET(CollectionClass, gain_selection),
                    NULL, NULL, g_cclosure_marshal_VOID__INT,
                    G_TYPE_NONE, 1, G_TYPE_INT);

    collection_signals[LOSE_SELECTION] = g_signal_new("lose_selection",
                    G_TYPE_FROM_CLASS(gclass), G_SIGNAL_RUN_LAST,
                    G_STRUCT_OFFSET(CollectionClass, lose_selection),
                    NULL, NULL, g_cclosure_marshal_VOID__INT,
                    G_TYPE_NONE, 1, G_TYPE_INT);

    collection_signals[SELECTION_CHANGED] = g_signal_new("selection_changed",
                    G_TYPE_FROM_CLASS(gclass), G_SIGNAL_RUN_LAST,
                    G_STRUCT_OFFSET(CollectionClass, selection_changed),
                    NULL, NULL, g_cclosure_marshal_VOID__INT,
                    G_TYPE_NONE, 1, G_TYPE_INT);

    g_object_class_install_property(gclass, PROP_VADJUSTMENT,
        g_param_spec_object("vadjustment", "Vertical Adjustment",
...
static void collection_init(GTypeInstance *instance, gpointer g_class)
{
	...
	/* GTK3: GtkDrawingArea supplies Collection with its own GdkWindow.
	 * The old no-window port could draw, but GTK could not reliably deliver
	 * button events to it inside GtkViewport. */
    gtk_widget_set_name(GTK_WIDGET(object), "rox-collection");
	...
	/* Keep one valid adjustment owned by Collection.  It is deliberately not
...
{
    return GTK_WIDGET(g_object_new(collection_get_type(), NULL));
}
                                           ...
                                           gint *minimum, gint *natural)
{
    if (minimum)
        *minimum = MIN_WIDTH;
    if (natural)
        *natural = MAX(MIN_WIDTH, (gint) COLLECTION(widget)->item_width);
}
...
{
    Collection *collection = COLLECTION(widget);
    gint height = collection_get_rows(collection) * collection->item_height;

    if (minimum)
        *minimum = MIN_HEIGHT;
    if (natural)
        *natural = MAX(MIN_HEIGHT, height);
}
                                     ...
                                     GtkAllocation *allocation)
{
    ...
    /* Chain to GtkDrawingArea so its GdkWindow is moved and resized too. */
    if (GTK_WIDGET_CLASS(parent_class)->size_allocate)
        GTK_WIDGET_CLASS(parent_class)->size_allocate(widget, allocation);
    else
        gtk_widget_set_allocation(widget, allocation);

    collection->columns = MAX(allocation->width / collection->item_width, 1);
...
static gboolean collection_draw(GtkWidget *widget, cairo_t *cr)
{
    Collection *collection = COLLECTION(widget);
    GtkStyleContext *context = gtk_widget_get_style_context(widget);
    GdkRectangle clip, item_area;
    int row, col, item;
    int start_row, last_row, start_col, last_col, phys_last_col;
    double x1, y1, x2, y2;

    cairo_clip_extents(cr, &x1, &y1, &x2, &y2);
    clip.x = MAX(0, (int) floor(x1));
    clip.y = MAX(0, (int) floor(y1));
    clip.width = MAX(1, (int) ceil(x2) - clip.x);
    clip.height = MAX(1, (int) ceil(y2) - clip.y);

    gtk_render_background(context, cr, clip.x, clip.y,
                          clip.width, clip.height);

    start_row = clip.y / collection->item_height;
    last_row = (clip.y + clip.height - 1) / collection->item_height;
    last_row = MIN(last_row, collection_get_rows(collection) - 1);

    start_col = clip.x / collection->item_width;
    phys_last_col = (clip.x + clip.width - 1) / collection->item_width;
    start_col = MIN(start_col, (int) collection->columns - 1);
    last_col = MIN(phys_last_col, (int) collection->columns - 1);
    ...
    for (row = start_row; row <= last_row; row++)
        for (col = start_col; col <= last_col; col++)
        {
            ...
            if (item == 0 || item < collection->number_of_items)
            {
                ...
                draw_one_item(collection, cr, item, &item_area);
            }
        }

    if (collection->lasso_box)
        draw_lasso_box(collection, cr);
                              ...
                              gpointer user_data)
{
    ...
    gtk_style_context_get_color(context,
            item->selected ? GTK_STATE_FLAG_SELECTED : GTK_STATE_FLAG_NORMAL,
            &colour);
    cairo_save(cr);
    gdk_cairo_set_source_rgba(cr, &colour);
    cairo_arc(cr, area->x + area->width / 2.0,
              area->y + area->height / 2.0,
              MIN(area->width, area->height) / 2.0 - 2.0,
              0.0, 2.0 * G_PI);
    cairo_fill(cr);
    cairo_restore(cr);
}
				    ...
				    GParamSpec   *pspec)
{
	...
	switch (prop_id)
	{
		case PROP_VADJUSTMENT:
			collection_set_vadjustment(collection,
...
void collection_set_vadjustment(Collection *collection,
                                GtkAdjustment *vadj)
{
	...
	switch (prop_id)
	{
		case PROP_VADJUSTMENT:
			g_value_set_object(value,
				collection_get_vadjustment(collection));
			break;
		default:
			G_OBJECT_WARN_INVALID_PROPERTY_ID(object,
							  prop_id, pspec);
		...
		case GDK_KEY_KP_Page_Up:
		  {
		        int first, last;
		       	get_visible_limits(collection, &first, &last);
			collection_move_cursor(collection, first - last + 1, 0, 0);
			break;
		  }
		...
		case GDK_KEY_KP_Page_Down:
		  {
		        int first, last;
		       	get_visible_limits(collection, &first, &last);
			collection_move_cursor(collection, last - first - 1, 0, 0);
			break;
		  }
		default:
			return FALSE;
...
static gint collection_scroll_event(GtkWidget *widget, GdkEventScroll *event)
{
	...
	if (diff)
	{
		double old_value = gtk_adjustment_get_value(collection->vadj);
		double new_value = 0;
		gboolean box = collection->lasso_box;
		double step = gtk_adjustment_get_page_increment(collection->vadj) / 2.0;

		new_value = CLAMP(old_value + diff * step, 0.0,
                MAX(0.0, gtk_adjustment_get_upper(collection->vadj)
                         - gtk_adjustment_get_page_size(collection->vadj)));
				    ...
				    guint32	 time)
{
	...
	g_return_if_fail(op == COLLECTION_SELECTION_SET || op == COLLECTION_SELECTION_INVERT);
	...
	for (y = area->y; y < area->y + area->height && y < rows; y++)
		for (x = area->x; x < area->x + area->width && x < cols; x++)
		{
			...
			if (item < collection->number_of_items)
			{
				if (op == COLLECTION_SELECTION_INVERT)
                                     ...
                                     GdkEventMotion *event)
{
    ...
    queue_lasso_redraw(collection);
    collection->drag_box_x[1] = (gint) event->x;
    collection->drag_box_y[1] = (gint) event->y;
    queue_lasso_redraw(collection);
    return TRUE;
}
...
static void add_lasso_box(Collection *collection)
{
    g_return_if_fail(!collection->lasso_box);
...
static void queue_lasso_redraw(Collection *collection)
{
    int x = MIN(collection->drag_box_x[0], collection->drag_box_x[1]);
    int y = MIN(collection->drag_box_y[0], collection->drag_box_y[1]);
    int width = ABS(collection->drag_box_x[1] - collection->drag_box_x[0]);
    int height = ABS(collection->drag_box_y[1] - collection->drag_box_y[0]);

    gtk_widget_queue_draw_area(GTK_WIDGET(collection),
...
static void scroll_to_show(Collection *collection, int item)
{
    ...
    else if (row >= last && gtk_widget_get_realized(GTK_WIDGET(collection)))
    {
        gint height = (gint) gtk_adjustment_get_page_size(collection->vadj);
        gtk_adjustment_set_value(collection->vadj,
...
static void get_visible_limits(Collection *collection, int *first, int *last)
{
    ...
    scroll = (gint) gtk_adjustment_get_value(collection->vadj);
    height = (gint) gtk_adjustment_get_page_size(collection->vadj);
    *first = MAX(scroll / collection->item_height, 0);
    *last = (scroll + MAX(height, 1) - 1) / collection->item_height;
    if (*last < *first)
        *last = *first;
}
...
static void cancel_wink(Collection *collection)
{
    gint item = collection->wink_item;

    g_return_if_fail(item != -1);
    collection->wink_item = -1;
    collection->wink_visible = FALSE;
    if (collection->wink_timeout)
    {
        g_source_remove(collection->wink_timeout);
        collection->wink_timeout = 0;
    }
...
static void invert_wink(Collection *collection)
{
    ...
    collection->wink_visible = !collection->wink_visible;
    collection_draw_item(collection, collection->wink_item, TRUE);
}
...
void collection_draw_item(Collection *collection, gint item, gboolean blank)
{
    ...
    gtk_widget_queue_draw_area(widget, area.x, area.y,
 ...
 * op selects set, invert or clear semantics without GdkFunction.
 */
void collection_end_lasso(Collection *collection, CollectionSelectionOp op)
{
    if (op != COLLECTION_SELECTION_CLEAR)
    {
        GdkRectangle region;

        find_lasso_area(collection, &region);
        collection_process_area(collection, &region, op, GDK_CURRENT_TIME);
    }
