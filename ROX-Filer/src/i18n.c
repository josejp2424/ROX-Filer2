...
void i18n_init(void)
{
	...
	/* Initialise the process locale before loading translations. */
	setlocale(LC_ALL, "");
 ...
 */
RoxItemFactoryEntry *translate_entries(RoxItemFactoryEntry *entries, gint n)
{
	guchar	*first = NULL, *second = NULL;	/* Previous menu, submenu */
	gint i;
	RoxItemFactoryEntry *ret;

	ret = g_malloc(sizeof(RoxItemFactoryEntry) * n);
	for (i = 0; i < n; i++)
	{
...
void free_translated_entries(RoxItemFactoryEntry *entries, gint n)
{
	gint i;
