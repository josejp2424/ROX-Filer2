...
typedef enum {
	...
	DESKTOP_TOOL_DRIVE_ICON_LAYOUT
} DesktopTool;
...
static gchar *find_application_directory(const gchar *argv0)
{
	...
	/* A normal AppDir binary sits beside AppInfo.xml. Meson intentionally keeps
	 * build products out of the source tree, so for the standard top-level
	 * build directory also accept ../ROX-Filer as the source AppDir. This lets
	 * ./build/ROX-Filer run directly without AppRun or an APP_DIR export, while
	 * packaged binaries continue to resolve resources from their own AppDir. */
	{
		gchar *appinfo = g_build_filename(directory, "AppInfo.xml", NULL);

		if (!g_file_test(appinfo, G_FILE_TEST_IS_REGULAR))
		{
			gchar *candidate = g_build_filename(directory, "..", "ROX-Filer", NULL);
			gchar *canonical = g_canonicalize_filename(candidate, NULL);
			gchar *candidate_appinfo = g_build_filename(canonical, "AppInfo.xml", NULL);

			if (g_file_test(candidate_appinfo, G_FILE_TEST_IS_REGULAR))
			{
				g_free(candidate_appinfo);
				g_free(candidate);
				g_free(appinfo);
				g_free(directory);
				return canonical;
...
int main(int argc, char **argv)
{
	...
	/* Rox-Filer2 must remain a normal foreground GTK process when it has to
	 * service the request itself.  Historical ROX daemonised with fork() after
	 * gtk_init(), but forking a GTK3 process after the display connection is
	 * open is unsafe on both X11 and Wayland and can leave background processes
	 * with no visible filer window.  Existing-instance behaviour is already
	 * handled above by remote_init(), so no post-GTK fork is needed here. */
	ROX_LOG_INFO("process",
	             "servicing request in foreground pid=%ld display=%s",
	             (long)getpid(),
	             gdk_display_get_default()
	                 ? G_OBJECT_TYPE_NAME(gdk_display_get_default()) : "none");
	...
	else if (desktop_tool != DESKTOP_TOOL_NONE)
	{
		...
		else if (desktop_tool == DESKTOP_TOOL_DRIVE_ICON_LAYOUT)
...
static void print_help_text(void)
{
	...
	g_print("      --desktop-drive-icon-layout\t%s\n",
		_("Arrange Drive Icons..."));
