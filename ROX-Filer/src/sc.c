...
static SmPropValue *make_card_value(gchar val)
{
	...
	value->value = g_memdup2(&val, sizeof(gchar));
	value->length = 1;
...
static SmClient *exit_client = NULL;

static void close_connection_at_exit(void)
{
	if (exit_client && exit_client->conn)
	{
		SmcCloseConnection(exit_client->conn, 0, NULL);
		exit_client->conn = NULL;
	}
}
                			...
                			Bool opening, IcePointer *watch_data)
{
	...
	if(opening)
	{
		...
		client->input_tag = rox_input_add(client->fd, ROX_INPUT_READ, &poll_ice_messages, client);
	}
	else
	{
...
#endif
			rox_input_remove(client->input_tag);
			sc_destroy(client);
		}
	}
}
...
gboolean sc_connect(SmClient *client)
{
	...
	gdk_x11_set_sm_client_id(client->id);
	XFree(client_id_ret);

	exit_client = client;
	atexit(close_connection_at_exit);
