...
OUTDIR="$(pwd)/rox-filer2-labwc-test-r81-${STAMP}"
mkdir -p "$OUTDIR"
...
# r81 manual regression hints.  DND is interactive, so the script cannot
# synthesize a compositor drag reliably; the technical log now records it.
echo ""
echo "Pruebas manuales r81:"
echo "  1) Clic derecho sobre un archivo: deben aparecer Cut / Copy / Paste."
echo "  2) Abra dos Rox-Filer2 con --debug y arrastre entre ventanas."
echo "     Debe aparecer Copy / Move / Link y el log debe contener category=dnd."

echo "  3) Mueva un icono del escritorio al centro y use Refresh Desktop: debe volver a la grilla."
echo "  4) Arrastre carpeta, .desktop, imagen y otros archivos entre dos Rox-Filer2: debe ofrecer Copiar/Mover/Enlace."
