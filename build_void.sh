#!/usr/bin/env bash
# Rox-Filer2 native Void Linux package builder.
# Compile Rox-Filer2 with Meson + libsmbclient and create a native .xbps package.
set -euo pipefail
...
Builds Rox-Filer2 natively on Void Linux with Samba/libsmbclient enabled and
creates a native .xbps package in output/. It also indexes output/ as a local
...
stage_runtime() {
    ...
    strip --strip-unneeded "$app_install/ROX-Filer" "$STAGE_DIR/usr/bin/rox-find"
...
make_void_package() {
    ...
    local deps=(gtk+3 glib libxml2 libSM libICE libX11 shared-mime-info smbclient hicolor-icon-theme desktop-file-utils util-linux file)
    local full_feature=(gtk-layer-shell udisks2 cifs-utils)
    local p
    ...
    # De-duplicate dependency names while preserving a stable order.
    local dep_string
    dep_string=$(printf '%s\n' "${deps[@]}" | awk '!seen[$0]++' | tr '\n' ' ')
